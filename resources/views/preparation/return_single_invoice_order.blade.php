    ...
    <div style="max-width:400px;margin:0 auto">
        ...
        @php
            ...
            echo ' <p style="text-align: center;display: block";font-size:20px>' .
                $barcode->getBarcodeSVG('{{$store->site_symbol}}r' . $return->return_number, 'C128', 3, 37, 'Black', false) .
                '</p>';
        @endphp

        <p> Return Placing Date: {{ date('Y-m-d', strtotime($return->created_at)) }}<br>
            Shipping Date : {{ date('Y-m-d') }} <br>
            Return Number : {{$store->site_symbol}}r{{ $return->return_number }} <br>
            Order Number : {{$store->site_symbol}}-{{ $return->order_number }} <br>
